...
const userSchema = require("../schemas/user.js"); // "../schemas/user.js" 파일에서 User 스키마를 가져온다.
...
router.post("/signup", async (req, res) => {
  ...
  const isExisUser = await userSchema.findOne({
    $or: [{ nickname }], // DB에서 nickname이 이미 존재하는지 확인한다.
  });
  if (isExisUser) {
  ...
  const user = new userSchema({ nickname, password }); // 새로운 User 인스턴스를 생성
  await user.save(); // User 인스턴스를 DB에 저장한다.
